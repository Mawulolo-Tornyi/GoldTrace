...
class Database:
    ...
    def save_prediction(self,result:dict):
        with self.connection() as con: con.execute('INSERT INTO predictions(ts,event_id,payload) VALUES(?,?,?)',(result.get('timestamp'),result.get('event_id'),json.dumps(result)))
    ...
    def latest_prediction(self):
        with self.connection() as con: row=con.execute('SELECT payload FROM predictions ORDER BY id DESC LIMIT 1').fetchone()
        return json.loads(row['payload']) if row else None
    def list_predictions(self,limit:int=100):
        with self.connection() as con: rows=con.execute('SELECT payload FROM predictions ORDER BY id DESC LIMIT ?',(limit,)).fetchall()
        return [json.loads(r['payload']) for r in rows]
    def get_prediction(self,event_id:str):
        with self.connection() as con: row=con.execute('SELECT payload FROM predictions WHERE event_id=? ORDER BY id DESC LIMIT 1',(event_id,)).fetchone()
        return json.loads(row['payload']) if row else None
    def save_sensor_reading(self, packet:dict):
        with self.connection() as con: con.execute('INSERT INTO sensor_readings(ts,node_id,payload) VALUES(?,?,?)',(packet.get('timestamp'),packet.get('node_id'),json.dumps(packet)))
    ...
    def list_alerts(self, limit:int=100):
        with self.connection() as con: rows=con.execute('SELECT id,payload FROM alerts ORDER BY id DESC LIMIT ?',(limit,)).fetchall()
        out=[]
        for r in rows:
            p=json.loads(r['payload']); p['alert_id']=r['id']; out.append(p)
        return out
    def get_alert(self, alert_id:int):
        with self.connection() as con: row=con.execute('SELECT id,payload FROM alerts WHERE id=?',(alert_id,)).fetchone()
    ...
    def acknowledge_alert(self,alert_id:int,by:str,notes:str=''):
        ...
        with self.connection() as con: con.execute('UPDATE alerts SET payload=? WHERE id=?',(json.dumps({k:v for k,v in item.items() if k!='alert_id'}),alert_id))
        return item
    ...
    def list_event_locations(self,limit:int=200):
        with self.connection() as con: rows=con.execute('SELECT event_id,payload FROM event_locations ORDER BY rowid DESC LIMIT ?',(limit,)).fetchall()
        return [{'event_id':r['event_id'],'location':json.loads(r['payload'])} for r in rows]
    def list_risk_zones(self,limit:int=200):
        with self.connection() as con: rows=con.execute('SELECT event_id,risk,payload FROM risk_zones ORDER BY id DESC LIMIT ?',(limit,)).fetchall()
    ...
    def update_alert_delivery(self,delivery_id:int,status:str,provider_response:str='',error:str|None=None):
        ...
        with self.connection() as con: con.execute('UPDATE alert_deliveries SET attempted_at=?,delivered_at=?,status=?,provider_response=?,error=? WHERE delivery_id=?',(now,delivered,status,provider_response,error,delivery_id))
    def list_alert_deliveries(self,event_id:str|None=None):
        ...
        with self.connection() as con: rows=con.execute(q,params).fetchall()
    ...
    def pending_alert_queue(self):
        with self.connection() as con: rows=con.execute("SELECT * FROM outbound_alert_queue WHERE status IN ('PENDING','RETRYING') ORDER BY queue_id").fetchall()
        return [dict(r) for r in rows]
    ...
    def mark_queue_exhausted(self,queue_id:int):
        with self.connection() as con: con.execute("UPDATE outbound_alert_queue SET status='FAILED' WHERE queue_id=?",(queue_id,))
    def record_agency_dispatch(self,event_id,risk,status,payload):
        with self.connection() as con: con.execute('INSERT INTO agency_dispatches(ts,event_id,risk,status,payload) VALUES(?,?,?,?,?)',(_now(),event_id,risk,status,json.dumps(payload)))
    def has_recent_agency_dispatch(self,event_id:str,risk:str)->bool:
        with self.connection() as con: row=con.execute("SELECT 1 FROM agency_dispatches WHERE event_id=? AND risk=? AND status IN ('SENT','SIMULATED','QUEUED') ORDER BY id DESC LIMIT 1",(event_id,risk)).fetchone()
        return bool(row)
    def list_agency_dispatches(self,limit:int=100):
        with self.connection() as con: rows=con.execute('SELECT * FROM agency_dispatches ORDER BY id DESC LIMIT ?',(limit,)).fetchall()
        out=[]
        for r in rows:
